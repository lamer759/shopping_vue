  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="a in addressInfo" :key="a.id">
        <span :class="{ selected: a.isDefault == 1 }" class="username">{{
          a.consignee
        }}</span>
        <p @click="changeDefault(a, addressInfo)">
      ...
      <div class="way">
        ...
        <div class="info clearFix">
          ...
          <p>配送时间:预计8月9(周三)09:00-15:00送达</p>
        </div>
      </div>
      <div class="detail">
        <h5>商品清单</h5>
        ...
        >
          <li>
            <img style="height: 100px" :src="order.imgUrl" />
          </li>
          <li>
            <p>{{ order.skuName }}</p>
            <h4>7天无理由退货</h4>
    ...
    <div class="sub clearFix">
      <!-- <router-link class="subBtn" to="/pay">提交订单</router-link> 不能直接跳转，之前要发请求，用a-->
      <a class="subBtn" @click="summitOrder">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    //最终提交订单的选中地址
    userDefaultAddress() {
      return this.addressInfo.find((i) => i.isDefault == 1) || {}; //返回的至少保证是个对象,不写|| {}会报错但不影响结果
    },
  },
  methods: {
    changeDefault(a, addressInfo) {
    ...
    async summitOrder() {
      ...
      let re = await this.$api.reqSubmit(tradeNo, data);
      console.log(re)
      if(re.code==200){
        this.orderId=re.data
        //路由跳转+路由传参
