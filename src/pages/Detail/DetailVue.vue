    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <ZoomVue :skuImageList="skuInfo.skuImageList || []"></ZoomVue>
          <!-- 小图列表 -->
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="squarr in spuSaleAttrList" :key="squarr.id">
                <dt class="title">{{ squarr.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: squlist.isChecked == 1 }"
                  v-for="squlist in squarr.spuSaleAttrValueList"
                  :key="squlist.id"
                  @click="changeActive(squlist, squarr.spuSaleAttrValueList)"
                >
                  {{ squlist.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
import ZoomVue from "./Zoom/ZoomVue.vue";
...
export default {
  ...
  methods: {
    changeActive(squlist, arr) {
      arr.forEach((element) => (element.isChecked = 0));
      squlist.isChecked = 1;
    },
    changeSkuNum(event) {
      //console.log(event)
    ...
    async addShopcar() {
      try {
        ...
        //需要传递skuinfo对象和skuNum数字，用query可以传递简单的数字，对象传过去地址栏很别扭
