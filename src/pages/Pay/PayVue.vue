...
export default {
  ...
  methods: {
    async getPayInfo() {
      let re = await this.$api.reqPayInfo(this.orderId);
      //console.log(re)
      if (re.code == 200) {
        this.payInfo = re.data;
      }
    },
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "请使用微信支付", {
        ...
        showClose: false,
        beforeClose:(type,_,done)=>{
          //type,取消或确定按钮，instance当前组件实例，done关闭弹窗的方法
          if(type=='cancel'){
            alert('请联系管理员')
      ...
      // let re= await this.$api.reqPayStatus(this.orderId)
      // console.log(re)
      if(!this.timer){
