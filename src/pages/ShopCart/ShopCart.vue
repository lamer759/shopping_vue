<template>
  <div class="cart">
      ...
      <div class="cart-body">
        ...
        >
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deletecar(cart)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
    ...
    <div class="cart-tool">
      ...
      <div class="option">
        <a @click="delateAllChecked">删除选中的商品</a>
  ...
  methods: {
    ...
    async handle(type, disNum, cart) {
      switch (type) {
        case "add":
          disNum = 1;
          cart.skuNum = cart.skuNum + 1;
          break;
        case "minus":
          cart.skuNum > 0 ? (disNum = -1) : (disNum = 0);
          cart.skuNum > 1 ? (cart.skuNum -= 1) : (cart.skuNum = 0);
          break;
        case "change":
    ...
    //删除商品
    async deletecar(cart) {
      try {
        await this.$store.dispatch("delateshopcatr", cart.skuId);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
    ...
    async updateChecked(cart, event) {
      //console.log(event.target.checked,cart)
      // let checked=event.target.checked ? '1':'0'
      // this.$store.dispatch('updatedChecked' ,{skuId:cart.skuId,isChecked:checked})
      // this.getData();
      try {
        let checked = event.target.checked ? "1" : "0";
        await this.$store.dispatch("updatedChecked", {
          skuId: cart.skuId,
          isChecked: checked,
        });
        this.getData();
    ...
    async delateAllChecked() {
      try {
        await this.$store.dispatch("delateAllChecked");
        this.getData()
      } catch (error) {
        alert(error.message)
      }
      //派发action，多调几次删除一个商品的接口
    },
   async updateAllChecked(event){
      //console.log(event)
      try {
        let ischecked = event.target.checked? '1':'0' //获取选择框的当前状态，并当做参数带给服务器
